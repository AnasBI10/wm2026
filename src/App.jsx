...
const teamFlag = n => ({
  ...
  "Portugal":"🇵🇹","Netherlands":"🇳🇱","Belgium":"🇧🇪","Mexico":"🇲🇽","Croatia":"🇭🇷",
  "Uruguay":"🇺🇾","Turkey":"🇹🇷","Morocco":"🇲🇦","Senegal":"🇸🇳","Colombia":"🇨🇴",
  "Australia":"🇦🇺","Switzerland":"🇨🇭","Serbia":"🇷🇸","Poland":"🇵🇱","South Korea":"🇰🇷",
  "Iran":"🇮🇷","Canada":"🇨🇦","Saudi Arabia":"🇸🇦","Cameroon":"🇨🇲","Tunisia":"🇹🇳",
  "South Africa":"🇿🇦","Ecuador":"🇪🇨","Peru":"🇵🇪","Chile":"🇨🇱","Venezuela":"🇻🇪",
  "Costa Rica":"🇨🇷","Japan":"🇯🇵","Ghana":"🇬🇭",
}[n] || "🏳️");
...
function AppInner() {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      // allorigins.win als CORS-Proxy — übergibt Headers NICHT weiter
      // Deshalb: API Key direkt als Query-Parameter (api-sports unterstützt das)
      const apiUrl = `https://v3.football.api-sports.io/fixtures?league=${WM_LEAGUE}&season=${WM_SEASON}`;
      ...
      catch { throw new Error("Ungültige API-Antwort"); }

      // API-Fehler abfangen (z.B. fehlender Token)
      if (json.errors && Object.keys(json.errors).length > 0) {
        throw new Error(`API: ${JSON.stringify(json.errors)}`);
      }

      if (!json.response || json.response.length === 0) {
        throw new Error("Keine Spiele in API-Antwort (WM 2026 noch nicht eingetragen)");
      }
  ...
  useEffect(() => {
    ...
    const iv = setInterval(fetchMatches, hasLive ? 30000 : 300000); // 30s live, 5min sonst
  ...
  const saveTip = async (match, home, away) => {
    ...
    try {
      await setDoc(doc(db,"tips",authUser.uid),
        { [match.id]: { home:h, away:a, savedAt: Date.now() } }, { merge: true });
    } catch(e) { notify("Fehler beim Speichern.", "error"); }
  };

  const saveBonusTip = async (question, answer) => {
    if (!authUser) return;
    if (isBonusLocked(question)) { notify("⏰ Bonusfrage gesperrt!", "error"); return; }
  ...
  const sendDuel = async (matchId, opponentId, amount) => {
    ...
    if (exists) { notify("Duell existiert bereits!", "warn"); return; }
    if (+amount < 1 || +amount > 500) { notify("Einsatz: 1–500€", "error"); return; }
    try {
      ...
      setDuelModal(null);
    } catch(e) { notify("Fehler beim Senden.", "error"); }
  };
  ...
  const leaderboard = Object.entries(allProfiles).map(([uid,p]) => {
    ...
    return { uid, ...p, pts:pts+bonusPts, matchPts:pts, bonusPts, exact, diff, tend, miss, total,
             duelWins:myDuelWins.length, duelEarned:myDuelWins.reduce((s,d)=>s+d.amount,0) };
      ...
      <header style={S.header}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={S.hSub}>
              {apiStatus==="error" ? <span style={{color:"#ef4444"}}>⚠️ API Fehler</span>
...
function AuthScreen({ notify }) {
  ...
  const go = async () => {
    ...
    try {
      if (mode==="register") {
        if (!ALLOWED_EMAILS.includes(cleanEmail)) return setErr("Diese E-Mail ist nicht zugelassen.");
      ...
      const map = {
        "auth/email-already-in-use":"E-Mail bereits registriert – einloggen!",
        ...
        "auth/network-request-failed":"Kein Internet.",
    ...
    <div style={S.authBg}><style>{GS}</style>
      ...
      <div style={S.authCard}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:12}}>
          <input style={S.field} type="email" placeholder="E-Mail" value={email} autoCapitalize="none" autoCorrect="off" spellCheck="false" onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()}/>
          <input style={S.field} type="password" placeholder="Passwort (min. 6 Zeichen)" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()}/>
        </div>
        {err&&<div style={S.errBox}>{err}</div>}
        <button style={{...S.authBtn,opacity:busy?0.7:1}} onClick={go} disabled={busy}>{busy?"…":mode==="login"?"Einloggen":"Account erstellen"}</button>
...
function OnboardingScreen({ authUser, onDone, notify }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      onDone(p);
    } catch(e) { notify("Fehler beim Speichern.", "error"); setBusy(false); }
  };
  return(
    <div style={S.authBg}><style>{GS}</style>
      <div style={S.authCard}>
        ...
        <input style={{...S.field,marginBottom:16,fontSize:20,textAlign:"center",fontWeight:800}} placeholder="Dein Spitzname…" value={name} maxLength={14} onChange={e=>setName(e.target.value)}/>
        <div style={{color:"#475569",fontSize:12,marginBottom:10,fontWeight:600}}>Wähle dein Emoji</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,justifyContent:"center",marginBottom:20}}>
          {EMOJIS.map(e=><button key={e} style={{...S.emojiBtn,...(emoji===e?S.emojiBtnA:{})}} onClick={()=>setEmoji(e)}>{e}</button>)}
        </div>
        <button style={{...S.authBtn,opacity:busy?0.7:1}} onClick={save} disabled={busy}>{busy?"…":"Los geht's! 🚀"}</button>
      </div>
    </div>
  );
}
...
function HomeTab({matches,leaderboard,myUid,profile,myTips,myBonus,duels,allProfiles,liveReacts,potTotal,openDuelCount,myBonusAnswered,onMatch,onDuel,tick}){
  ...
  return(
    <div style={S.page}>
      ...
      {bonusLeft>0&&<div style={{...S.alert,borderColor:"rgba(139,92,246,0.35)",background:"rgba(139,92,246,0.08)"}}>
        <span style={{fontSize:26}}>🎯</span>
        <div style={{flex:1}}><div style={{fontWeight:800,color:"#8b5cf6",fontSize:15}}>{bonusLeft} Bonusfragen offen!</div><div style={{color:"#64748b",fontSize:12}}>Vor Anpfiff beantworten</div></div>
      </div>}
      {openDuelCount>0&&<div style={S.alert}>
        <span style={{fontSize:26}}>🥊</span>
        <div style={{flex:1}}><div style={{fontWeight:800,color:"#ef4444",fontSize:15}}>{openDuelCount}x Duell-Anfrage!</div><div style={{color:"#64748b",fontSize:12}}>Jemand fordert dich heraus</div></div>
      </div>}
...
function MatchCard({m,myTips,liveReacts,onMatch,onDuel,tick}){
  ...
  const fmtDate=(iso)=>{try{const d=new Date(iso);if(isNaN(d))return iso;return d.toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"})+" "+d.toLocaleTimeString("de-DE",{hour:"2-digit",minute:"2-digit"});}catch{return iso;}};
  return(
    <div style={{...S.mc,...(locked&&!m.result&&!m.live?{opacity:0.6}:{})}} className="mc" onClick={()=>onMatch(m)}>
      ...
      <div style={S.mcMeta}>
        ...
        {m.live?<span style={S.liveBadge}>🔴 {m.elapsed}'</span>:<span style={{color:isUrgent?"#ef4444":"#334155",fontSize:11,fontWeight:isUrgent?800:400}}>{fmtDate(m.date)}</span>}
      ...
      <div style={S.mcRow}>
        <div style={S.teamC}>
          {m.homeLogo?<img src={m.homeLogo} style={{width:34,height:34,objectFit:"contain"}} alt="" onError={e=>{e.target.style.display="none";}}/>:<span style={{fontSize:32}}>{m.homeFlag}</span>}
        ...
        <div style={S.scoreMid}>
          ...
          {!m.live&&!m.result&&(myTip?<div style={S.tipBub}>{myTip.home}:{myTip.away}</div>:<div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:"#1e293b",letterSpacing:1}}>VS</div>)}
        </div>
        <div style={{...S.teamC,alignItems:"flex-end"}}>
          {m.awayLogo?<img src={m.awayLogo} style={{width:34,height:34,objectFit:"contain"}} alt="" onError={e=>{e.target.style.display="none";}}/>:<span style={{fontSize:32}}>{m.awayFlag}</span>}
  ...
  return(
    <div style={S.page}>
      ...
      <div style={S.filterRow}>
        {groups.map(g=><button key={g} style={{...S.fb,...(filter===g?S.fbA:{})}} onClick={()=>setFilter(g)}>{g==="Alle"?"Alle":g.length>6?g.slice(0,8)+"…":g}</button>)}
      </div>
...
function BonusTab({questions,myBonus,bonusResults,allProfiles,bonusTips,onSave,tick}){
  ...
  return(
    <div style={S.page}>
      ...
      {questions.map(q=>(
        <BonusQuestion key={q.id} q={q} myAnswer={myBonus[q.id]} correctAnswer={bonusResults[q.id]}
  ...
  return(
    <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${borderColor}`,borderRadius:16,padding:"14px 15px",marginBottom:10}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
        ...
        {locked?<span style={{background:"rgba(100,116,139,0.15)",color:"#64748b",borderRadius:6,padding:"2px 8px",fontSize:10,fontWeight:800,flexShrink:0,marginLeft:8}}>🔒</span>
      ...
      {correctAnswer&&<div style={{background:isCorrect?"rgba(34,197,94,0.1)":"rgba(239,68,68,0.08)",border:`1px solid ${isCorrect?"rgba(34,197,94,0.3)":"rgba(239,68,68,0.2)"}`,borderRadius:10,padding:"8px 12px",marginBottom:10,fontSize:13}}>
      ...
      {!locked&&!correctAnswer&&<div style={{marginBottom:8}}>
        {q.type==="number"&&<div style={{display:"flex",gap:8}}>
          <input type="number" style={{...S.field,flex:1,fontSize:20,textAlign:"center",fontWeight:900,padding:"10px"}} value={val} placeholder="Zahl…" onChange={e=>setVal(e.target.value)}/>
          <button style={S.saveBtn} onClick={()=>val!=""&&onSave(val)}>✓</button>
        </div>}
        {q.type==="single_choice"&&<div style={{display:"flex",flexDirection:"column",gap:6}}>
          {q.options.map(o=><button key={o} style={{background:val===o?"rgba(139,92,246,0.2)":"rgba(255,255,255,0.04)",border:`1px solid ${val===o?"#8b5cf6":"rgba(255,255,255,0.08)"}`,borderRadius:10,padding:"10px 14px",color:val===o?"#c4b5fd":"#94a3b8",fontWeight:700,fontSize:14,cursor:"pointer",textAlign:"left"}} onClick={()=>{setVal(o);onSave(o);}}>{val===o?"✓ ":""}{o}</button>)}
        </div>}
        {(q.type==="team_select"||q.type==="player_select"||q.type==="keeper_select")&&<div>
          <div style={{display:"flex",gap:8,marginBottom:8}}>
            <input style={{...S.field,flex:1,padding:"8px 12px",fontSize:13}} placeholder="Suchen…" value={search} onChange={e=>setSearch(e.target.value)}/>
      ...
      {(locked||correctAnswer)&&myAnswer&&<div style={{background:"rgba(139,92,246,0.08)",border:"1px solid rgba(139,92,246,0.2)",borderRadius:8,padding:"7px 12px",fontSize:13,color:"#c4b5fd",fontWeight:700}}>Deine Antwort: {myAnswer} {q.type==="team_select"?teamFlag(myAnswer):""}</div>}
      {locked&&!myAnswer&&<div style={{color:"#334155",fontSize:12,fontStyle:"italic"}}>Nicht beantwortet — 0 Punkte</div>}
      {allAnswers.filter(a=>a.answer).length>0&&<button style={{marginTop:8,background:"none",border:"none",color:"#475569",fontSize:12,cursor:"pointer",padding:0}} onClick={()=>setOpen(!open)}>{open?`▲ Verbergen`:`▼ Alle Antworten (${allAnswers.filter(a=>a.answer).length})`}</button>}
      {open&&<div style={{marginTop:8,display:"flex",flexDirection:"column",gap:4}}>
        {allAnswers.filter(a=>a.answer).map(a=>(
          <div key={a.uid} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"6px 10px"}}>
            ...
            <span style={{fontSize:13,fontWeight:700,color:correctAnswer?(String(a.answer)===String(correctAnswer)?"#22c55e":"#ef4444"):"#f59e0b"}}>{a.answer} {q.type==="team_select"?teamFlag(a.answer):""}</span>
    ...
    <div style={S.page}>
      ...
      <div style={S.legend}>
        {[["#22c55e","🏆 Platz 1–2 · Gewinner"],["#64748b","😐 Platz 3–4 · Mittelfeld"],["#ef4444","🍺 Platz 5–7 · Zahlt Runde"]].map(([c,t])=>(
          <div key={c} style={{display:"flex",alignItems:"center",gap:8,color:"#475569",fontSize:12}}><div style={{width:10,height:10,borderRadius:"50%",background:c,flexShrink:0}}/>{t}</div>
        ))}
      </div>
      <div style={S.rulesBox}>
        ...
        {[["#22c55e","4 Pts","Exaktes Ergebnis 🎯"],["#3b82f6","3 Pts","Tordifferenz richtig 📐"],["#f59e0b","2 Pts","Richtiger Gewinner ↗"],["#ef4444","0 Pts","Falsch ✗"],["#8b5cf6","bis 15","Bonusfragen 🎯"]].map(([c,p,d])=>(
          <div key={p} style={{display:"flex",gap:10,marginBottom:6,alignItems:"center"}}><span style={{color:c,fontWeight:900,width:60,fontSize:12}}>{p}</span><span style={{color:"#64748b",fontSize:12}}>{d}</span></div>
        ))}
      </div>
    </div>
  );
...
function DuelleTab({duels,allProfiles,myUid,matches,onNewDuel}){
  ...
  return(
    <div style={S.page}>
      ...
      {openForMe.length>0&&<><SectionTitle>🔔 Deine Anfragen</SectionTitle>
        {openForMe.map(([key,d])=>{const c=allProfiles[d.challengerId],m=matches.find(x=>x.id===d.matchId);return(
          <div key={key} style={{...S.duelC,border:"1px solid rgba(239,68,68,0.3)"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{color:"#64748b",fontSize:12}}>{m?.home} vs {m?.away}</span><span style={{background:"rgba(245,158,11,0.14)",color:"#f59e0b",borderRadius:8,padding:"2px 10px",fontWeight:900}}>{d.amount}€</span></div>
            <div style={{fontWeight:700,color:"#f1f5f9"}}>{c?.emoji} {c?.name} fordert dich heraus!</div>
          </div>
        );})}
      </>}
      <SectionTitle>➕ Neues Duell</SectionTitle>
      {availableMatches.length>0?<div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:8,marginBottom:14}}>
        {availableMatches.map(m=>(
          <button key={m.id} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"12px 14px",display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer",flexShrink:0,minWidth:76}} onClick={()=>onNewDuel(m)}>
            <span style={{fontSize:22}}>{m.homeFlag}</span><span style={{color:"#334155",fontSize:10,fontWeight:700}}>vs</span><span style={{fontSize:22}}>{m.awayFlag}</span><span style={{color:"#f59e0b",fontSize:9,fontWeight:800}}>🥊 DUELL</span>
          </button>
        ))}
      </div>:<div style={S.empty}>Keine offenen Spiele</div>}
      <SectionTitle>Alle Duelle</SectionTitle>
      {Object.keys(duels).length===0&&<div style={S.empty}>Noch keine Duelle!</div>}
        ...
        return(
          <div key={key} style={{...S.duelC,border:isMe?"1px solid rgba(245,158,11,0.2)":"1px solid rgba(255,255,255,0.06)"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <div style={{display:"flex",flexDirection:"column",gap:2}}><span style={{fontWeight:700,fontSize:13}}>{c?.emoji} {c?.name}</span>{d.challengerTip&&<span style={{background:"rgba(245,158,11,0.14)",color:"#f59e0b",borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:900,width:"fit-content"}}>{d.challengerTip.home}:{d.challengerTip.away}</span>}</div>
              <span style={{color:"#f59e0b",fontSize:18,fontWeight:900}}>🥊</span>
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:2}}><span style={{fontWeight:700,fontSize:13}}>{o?.emoji} {o?.name}</span>{d.challengedTip?<span style={{background:"rgba(245,158,11,0.14)",color:"#f59e0b",borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:900}}>{d.challengedTip.home}:{d.challengedTip.away}</span>:<span style={{color:"#334155",fontSize:10}}>ausstehend…</span>}</div>
            </div>
  ...
  return(
    <div style={S.page}>
      <div style={{background:"radial-gradient(ellipse at 50% 0%,rgba(245,158,11,0.16),rgba(10,15,28,1) 65%)",border:"1px solid rgba(245,158,11,0.22)",borderRadius:24,padding:"28px 24px",textAlign:"center",marginBottom:16,position:"relative",overflow:"hidden"}}>
        ...
        <div style={{background:"rgba(0,0,0,0.3)",borderRadius:12,padding:"10px 20px",display:"inline-flex",flexDirection:"column",gap:4,minWidth:180}}>
          {[["Grundbeiträge",potTotal+"€","#f1f5f9"],["Duelle","+"+duelPot+"€","#f59e0b"]].map(([l,v,c])=>(
            <div key={l} style={{display:"flex",justifyContent:"space-between",gap:20,fontSize:13}}><span style={{color:"#64748b"}}>{l}</span><span style={{color:c,fontWeight:800}}>{v}</span></div>
          ))}
        </div>
      </div>
      ...
      <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:"4px 14px",marginBottom:14}}>
        {Object.entries(allProfiles).map(([uid,p])=>{const pay=payments[uid];return(
          <div key={uid} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:"1px solid rgba(255,255,255,0.04)",background:uid===myUid?"rgba(245,158,11,0.05)":"transparent"}}>
            ...
            <span style={pay?.verified?S.greenBadge:pay?.paid?S.yellowBadge:S.redBadge}>{pay?.verified?"✓ Bestätigt":pay?.paid?"⏳ Ausstehend":"✕ Offen"}</span>
          </div>
        );})}
      </div>
      {!myPay?.verified&&!myPay?.paid&&<button style={{display:"flex",flexDirection:"column",alignItems:"center",background:"#003087",color:"#fff",borderRadius:16,padding:"16px",border:"none",cursor:"pointer",width:"100%",gap:3}} onClick={initPay}>
  ...
  return(
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={e=>e.stopPropagation()}>
        ...
        {!locked&&!isDone&&<div style={{display:"flex",alignItems:"center",gap:8,justifyContent:"center",marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:4}}>
          {Object.entries(allProfiles).map(([uid,p])=>{const t=allTips[uid]?.[m.id],pts=calcPoints(t,m.result);return(
            <div key={uid} style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:12,padding:"10px 12px",display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{border:`1px solid ${t?"#f59e0b":"#1e293b"}`,borderRadius:8,padding:"3px 8px",fontSize:14,fontWeight:900,color:t?"#f59e0b":"#334155",flexShrink:0}}>{t?`${t.home}:${t.away}`:"–"}</span>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4}}>
          {REACTIONS.map(e=><button key={e} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:20,padding:"7px 11px",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",gap:4}} onClick={()=>onReact(e)}>{e}{reactions[e]>0&&<span style={{color:"#64748b",fontSize:11,fontWeight:700}}>{reactions[e]}</span>}</button>)}
        </div>
        {matchDuels.length>0&&<><div style={S.mSection}>Duelle</div>
          {matchDuels.map(([key,d])=>{const c=allProfiles[d.challengerId],o=allProfiles[d.opponentId],sC=d.status==="settled"?"#22c55e":"#f59e0b";return(
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
          {others.map(([uid,p])=>(
            <button key={uid} style={{background:opp===uid?"rgba(245,158,11,0.14)":"rgba(255,255,255,0.04)",border:`1px solid ${opp===uid?"#f59e0b":"rgba(255,255,255,0.08)"}`,borderRadius:12,padding:"12px 6px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4}} onClick={()=>setOpp(uid)}>
              <span style={{fontSize:22}}>{p.emoji||"⚽"}</span><span style={{fontSize:12,fontWeight:800,color:"#f1f5f9"}}>{p.name}</span>
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:10,flexWrap:"wrap"}}>
          {[5,10,15,20,50].map(v=><button key={v} style={{background:amount===v?"rgba(245,158,11,0.18)":"rgba(255,255,255,0.05)",border:`1px solid ${amount===v?"#f59e0b":"rgba(255,255,255,0.08)"}`,color:amount===v?"#f59e0b":"#94a3b8",borderRadius:10,padding:"8px 14px",cursor:"pointer",fontWeight:800,fontSize:14}} onClick={()=>setAmount(v)}>{v}€</button>)}
        </div>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button style={{...S.saveBtn,flex:1,padding:14,fontSize:15,opacity:opp?1:0.4}} disabled={!opp} onClick={()=>opp&&onSend(m.id,opp,amount)}>Anfrage senden 🥊</button>
        </div>
      </div>
    </div>
  );
...
function Splash(){return(<div style={{minHeight:"100vh",background:"#060b14",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}><style>{GS}</style><span style={{fontSize:64,animation:"pulse 1s infinite"}}>⚽</span><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,color:"#f59e0b",letterSpacing:3}}>WM 2026</div></div>);}
function SectionTitle({children}){return <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:1,color:"#f1f5f9",marginBottom:10,marginTop:4}}>{children}</div>;}
